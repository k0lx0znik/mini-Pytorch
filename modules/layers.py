...
class Linear(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        output_grad = grad_output @ self.weight
        return output_grad
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            self.var = np.var(input, ddof=0, axis=0)
        ...
        if self.affine:
            batch_norm = self.norm_input * self.weight + self.bias
        else:
            batch_norm = self.norm_input
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:    
            batch_norm_grad = (1/len(input)) * self.inv_sqrt_var * (len(input) * grad_output -
                            np.sum(grad_output, axis=0) -
                            self.input_mean * (self.inv_sqrt_var ** 2) * np.sum(grad_output * self.input_mean, axis=0))
        else:
            batch_norm_grad = grad_output * self.inv_sqrt_var 
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training == True:
            ...
            dropout = self.mask * input * (1/(1-self.p))
        else:
            dropout = input
        return dropout
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        if self.training:
            dropout_grad = self.mask * grad_output * (1/(1-self.p))
        else:
