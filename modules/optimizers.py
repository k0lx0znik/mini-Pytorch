...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            ...
            """
            g_t = grad + self.weight_decay * param
            
            np.add(self.momentum * m, g_t, out=m)
            
            if self.nesterov:
                update = self.momentum * m + g_t
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            np.add(self.beta1 * m, (1 - self.beta1) * g_t, out=m)
            
            np.add(self.beta2 * v, (1 - self.beta2) * (g_t ** 2), out=v)
